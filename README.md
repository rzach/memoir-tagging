...
[`memoir-tagging.dtx/cls`](./memoir-tagging.dtx) is the [*memoir*
...
- The code for `\part` and `\chapter` is suppressed in this way.
  [memfirstaid-headings.sty](./memfirstaid-headings.sty) contains
  experimental code that makes use of the new [heading
  ...
  instead of *memoir*'s own definitions. The edits try to mimic the
  effect of *memoir*'s configuration commands as much as I could figure
  (e.g., use `\chaptitlefont`) so that chapter styles etc. still more
  or less work. More sophisticated chapter formats will probably need
  their own templates. **This requires the most recent versions of the
  heading templates, so `lualatex-dev`.**
...
- For setting page dimensions and layout, the
  [*geometry*](https://ctan.org/pkg/geometry) package can do a lot of
  the same things that *memoir*'s options can. It is not as convenient
  as *memoir* and doesn't provide as many presets (paper sizes,
  layouts). It uses different terminology than *memoir*'s (which uses
  traditional printer's and typesetter's terminology): e.g.,
  geometry's paper = *memoir*'s trim, layout/body = page, inner =
  spine, outer = edge, etc. However, *memoir* types out the layout
  dimensions on every run (on the console only, not in the log) and
  you could just use what *memoir* tells you there for geometry.
...
- Changing the chapter and other heading styles can be done by editing
  ...
  [memfirstaid-headings.sty](./memfirstaid-headings.sty). An
  alternate package that provides functionality to define chapter
  styling and is compatible with tagging is [*fncychap*](https://ctan.org/pkg/fncychap).
